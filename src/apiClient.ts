  ...
  switch (category) {
    case ImageCategory.COVER: {
      if (['brand-ambassador', 'brand-ambassador-female', 'brand-ambassador-male'].includes(style)) {
        ...
        return `Thai Marketplace Brand Ambassador product cover for "${productData.name}". Use exactly ${presenter} who naturally holds, uses, or introduces the exact reference product. The product must be the main purchase object: large, sharp, unobstructed, and faithful to the reference in shape, logo, colour, materials, proportions, labels, and included pieces. DYNAMIC CREATIVE TYPOGRAPHY & TEXT ART: Design original, eye-catching Thai visual headline graphics. Feel free to vary the font style (e.g. bold 3D extruded lettering, modern Thai sans-serif, brush typography, glowing outline, slanted dynamic headline, ribbon banner tag, or stylized badge container). Match the text color palette dynamically to the product and campaign theme (e.g. warm gold, electric blue, energetic orange, neon accent, or luxury dark). Do NOT limit to simple repeated rounded pill capsules. ${variation} On every regeneration, experiment with a distinct typography layout, font style, badge geometry, and text color combination, while preserving the same young attractive brand-ambassador character, product identity, and high-converting Shopee commercial aesthetic.`;
      }
      return `High-impact commercial E-Commerce COVER image (ภาพปกสินค้า) for "${productData.name}". Product Description: ${productData.description}. Visual Style: ${style}. Product is large and unmistakable in the foreground. DYNAMIC THAI TYPOGRAPHY DESIGN: Main Thai headline designed with creative commercial font art, 3D text effects, stylized banner tags, or dynamic color accents matching the visual style. All text MUST be in THAI LANGUAGE ONLY (ภาษาไทยเท่านั้น). Professional studio lighting, sharp product details.`;
    }

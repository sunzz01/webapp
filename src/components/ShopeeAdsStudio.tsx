...
export type ThaiAdsCard = ShopeeAdBrief & { id: string; status: AdStatus; imageUrl?: string; error?: string; modelUsed?: string; promptUsed?: string };
...
export function ShopeeAdsStudio({ dark, imageModel, session, setSession }: {
  ...
  const regenerate = async (card: ThaiAdsCard) => {
    ...
    try { const result = await generateProductImage(categoryForCard(card.id), { name, description: details, features: confirmedFacts, images: allImages }, 'shopee', buildThaiAdsPrompt(card), imageModel, '1:1'); updateCard(card.id, { status: 'completed', imageUrl: result.imageUrl, thaiCopy: card.thaiCopy.length ? card.thaiCopy : result.thaiTexts, modelUsed: result.modelUsed, promptUsed: result.promptUsed }); }
    catch (error) { updateCard(card.id, { status: 'error', error: error instanceof Error ? error.message : 'สร้างภาพไม่สำเร็จ' }); }
  };

...
export function ShopeeAdsStudio({ dark, seed, imageModel }: { dark: boolean; seed?: ThaiAdsSeed | null; imageModel: string }) {
  ...
  const [cards, setCards] = useState<AdCard[]>([]); const [isGenerating, setIsGenerating] = useState(false); const [notice, setNotice] = useState('');
  ...
  const updateCard = (id: string, patch: Partial<AdCard>) => setCards(prev => prev.map(card => card.id === id ? { ...card, ...patch } : card));

  const generate = async () => {
    if (!allImages.length) return setNotice('เพิ่มภาพสินค้าหลักอย่างน้อย 1 ภาพก่อนสร้าง');
    if (!name.trim()) return setNotice('ใส่ชื่อสินค้าก่อนเริ่มสร้าง');
    const work = createCards(); setCards(work); setNotice('กำลังเตรียมคิวสร้างภาพ…'); setIsGenerating(true);
    ...
    setIsGenerating(false); setNotice('สร้างภาพครบคิวแล้ว คุณแก้ข้อความและสร้างใหม่เป็นรายภาพได้');
  };
  ...
  const downloadZip = async () => { const complete = cards.filter(card => card.imageUrl); if (!complete.length) return; setNotice('กำลังเตรียม ZIP พร้อมข้อความไทย…'); const zip = new JSZip(); for (const card of complete) zip.file(`${cleanName(name)}-${card.id}.png`, await imageWithCopy(card.imageUrl!, card.thaiCopy)); const blob = await zip.generateAsync({ type: 'blob' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${cleanName(name)}-shopee-ads.zip`; a.click(); URL.revokeObjectURL(a.href); setNotice('ดาวน์โหลด ZIP เรียบร้อย'); };
  ...
  return <section className="max-w-7xl mx-auto space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <div className={`rounded-3xl border p-6 shadow-sm ${classCard}`}>
        ...
        {allImages.length > 0 && <div className="mt-4 flex gap-2 overflow-x-auto">{allImages.map((src, i) => <div key={src} className="relative shrink-0"><img src={src} className="h-16 w-16 rounded-xl object-cover"/><button onClick={() => setAssets(prev => ({ product: prev.product.filter(x => x !== src), package: prev.package.filter(x => x !== src), logo: prev.logo.filter(x => x !== src) }))} className="absolute -right-1 -top-1 rounded-full bg-slate-900 p-1 text-white"><X size={11}/></button></div>)}</div>}
        <div className="mt-5 grid gap-4"><label className="text-sm font-bold">ชื่อสินค้า<input value={name} onChange={e => setName(e.target.value)} placeholder="เช่น เครื่องชั่งดิจิทัล รุ่น X1" className="mt-1 w-full rounded-xl border border-slate-300 bg-transparent px-3 py-2.5 outline-none focus:border-orange-500"/></label><label className="text-sm font-bold">รายละเอียดสินค้า<textarea value={details} onChange={e => setDetails(e.target.value)} rows={3} placeholder="บอกวัสดุ กลุ่มลูกค้า การใช้งาน หรือบริบทที่ต้องการ" className="mt-1 w-full rounded-xl border border-slate-300 bg-transparent px-3 py-2.5 outline-none focus:border-orange-500"/></label><label className="text-sm font-bold">ข้อเท็จจริงที่ยืนยันแล้ว <span className="font-normal text-slate-500">(หนึ่งข้อ/บรรทัด)</span><textarea value={factsText} onChange={e => setFactsText(e.target.value)} rows={5} placeholder={'เช่น รองรับน้ำหนักสูงสุด 10 กก.\nหน้าจอ LED\nในกล่องมีถาด 1 ชิ้น'} className="mt-1 w-full rounded-xl border border-slate-300 bg-transparent px-3 py-2.5 outline-none focus:border-orange-500"/></label></div>
      </div>

      <div className={`rounded-3xl border p-6 shadow-sm ${classCard}`}><h3 className="font-black text-xl">2. วางแผนชุดภาพ</h3><div className="mt-5"><p className="text-sm font-bold">จำนวนภาพ</p><div className="mt-2 flex flex-wrap gap-2">{[4, 6, 8, 10].map(value => <button key={value} onClick={() => setCount(value)} className={`rounded-xl px-4 py-2 text-sm font-bold ${count === value ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-200'}`}>{value} ภาพ</button>)}</div></div><div className="mt-6 rounded-2xl border border-orange-200 bg-orange-50 p-4 dark:bg-orange-950/20"><label className="flex cursor-pointer items-center gap-3"><input type="checkbox" checked={heroWithPerson} onChange={e => setHeroWithPerson(e.target.checked)} className="h-4 w-4 accent-orange-500"/><span className="font-bold"><UserRound className="mr-1 inline" size={17}/>ภาพปกมีคนกำลังใช้งานสินค้า</span></label>{heroWithPerson && <input value={personBrief} onChange={e => setPersonBrief(e.target.value)} className="mt-3 w-full rounded-xl border border-orange-200 bg-white px-3 py-2 text-sm text-slate-800"/>}</div><div className="mt-6 space-y-2">{BLUEPRINTS.slice(0, count).map((item, index) => <div key={item.id} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-800"><span className="w-6 text-xs font-black text-orange-500">{String(index + 1).padStart(2, '0')}</span><span className="text-sm font-semibold">{item.title}</span>{item.includePerson && <UserRound className="ml-auto text-orange-500" size={16}/>}</div>)}</div><button disabled={isGenerating} onClick={generate} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 py-3.5 font-black text-white shadow-lg shadow-orange-500/25 disabled:opacity-50">{isGenerating ? <Loader2 className="animate-spin"/> : <Sparkles/>}{isGenerating ? 'กำลังสร้างภาพตามคิว…' : `สร้างชุดภาพ ${count} ภาพ`}</button>{notice && <p className="mt-3 text-center text-sm text-slate-500">{notice}</p>}</div>
    </div>

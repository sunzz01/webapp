...
export function generateStructuredPrompt(productName: string, style: string): string {
  ...
  const styleGuides: Record<string, { composition: string; styleElements: string; negative: string }> = {
    ...
    'tiktok': {
      composition: 'vertical 9:16 social commerce layout, product hero 55% lower frame, bold hook text upper third',
      styleElements: `
        - Background: dark gradient (#010101 to #121212) with subtle cyan-pink light leaks (#00F2EA, #FF0050)
        - Top hook text: oversized bold caption like "ต้องมี!" or "ราคานี้ห้ามพลาด" with white outline + drop shadow
        - TikTok Shop badge: "TikTok Shop" pill + shopping bag icon, cyan accent
        - Product: dynamic angle, slight tilt, creator-UGC energy (not stiff studio catalog)
        - Price sticker: bright contrasting tag "ลด 50%" or strikethrough price → sale price
        - Social proof row: hearts icon, "12.4K likes", comment bubble, share arrow (semi-transparent UI chrome)
        - Hashtag chips: #TikTokShop #FlashSale #ของดีบอกต่อ (small rounded pills)
        - Bottom CTA bar: "Shop Now" / "ซื้อเลย" button in TikTok pink (#FF0050)
        - Optional: trending sound wave graphic, "For You" style feed frame edges
        - Typography: bold condensed sans-serif, high contrast, mobile-native
        - Safe zones: keep key text inside center 80% for 9:16 crop
      `,
      negative: 'horizontal catalog layout, white ecommerce studio, corporate B2B, muted pastel, no text overlays, static Amazon-style listing'
    },
    'tiktok02': {
      composition: 'creator POV vertical frame, product in hand 40%, face/hands optional, feed UI overlays',
      styleElements: `
        - Mock "For You" page chrome: top tabs, side action icons (like, comment, share) on right edge
        - Creator hand holding product toward camera, authentic smartphone-shot feel
        - Viral hook headline top-center: 1-2 lines max, Thai or English, bold white with black stroke
        - Flash deal sticker overlapping product corner (pink/cyan burst)
        - Floating comment: "อยากได้มาก!" style chat bubble
        - TikTok Shop cart icon + "เพิ่มลงตะกร้า" micro-copy
        - Slight motion blur or speed lines for energy
        - Color: black background, neon cyan + hot pink accents only
      `,
      negative: 'professional studio packshot, landscape banner, minimal white background, luxury editorial'
    },
    'lazada-flagship': {
...
function buildCategoryTaskPrompt(
  ...
  switch (category) {
    case ImageCategory.COVER: {
      ...
      if (['tiktok', 'tiktok02'].includes(style.toLowerCase())) {
        prompt += '\n\nFORMAT: Vertical 9:16 TikTok Shop / Reels placement — mobile-first, bold hook text, Shop Now CTA.';
      }
      return prompt;
    }
